...
function startGasAnimation() {
  setInterval(() => {
    DummyData.items.forEach((item, index) => {
      const next1 = item.gas1[gasIndex % item.gas1.length];
      const next2 = item.gas2[gasIndex % item.gas2.length];
      const next3 = item.gas3[gasIndex % item.gas3.length];
      const next4 = item.gas4[gasIndex % item.gas4.length];
      ...
      const g = window.liveGauges?.[index];
      const canvases = {
        NH3: document.getElementById(`ammoniaGauge_${index}`),
        H2S: document.getElementById(`hydrogenSulfideGauge_${index}`),
        CH4: document.getElementById(`methaneGauge_${index}`),
        CO2: document.getElementById(`carbonDioxideGauge_${index}`),
      };

      if (g?.NH3) setGaugeValue(g.NH3, canvases.NH3, next1);
      if (g?.H2S) setGaugeValue(g.H2S, canvases.H2S, next2);
      if (g?.CH4) setGaugeValue(g.CH4, canvases.CH4, next3);
      if (g?.CO2) setGaugeValue(g.CO2, canvases.CO2, next4);
    });

    if (window.applyAllAtmosphereDummyData) {
      window.applyAllAtmosphereDummyData();
    }

    if (window.renderGaugesFromSelectedPoint) {
      window.renderGaugesFromSelectedPoint();
    }
...
function setGaugeValue(gauge, canvas, value) {
