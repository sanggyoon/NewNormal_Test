...
const gasNames = ['NH3', 'H2S', 'CH4', 'CO2'];
const canvasIds = [
  'ammoniaGauge_',
  'hydrogenSulfideGauge_',
  'methaneGauge_',
  'carbonDioxideGauge_',
];
...
for (let i = 0; i < 4; i++) {
  ...
  gasKeys.forEach((key, idx) => {
    document.querySelectorAll(`#${canvasIds[idx]}${i}`).forEach((canvas) => {
      ...
      gauge.set(value);
      window.liveGauges[i][gasNames[idx]] = gauge; // 저장
    });
  });
}
